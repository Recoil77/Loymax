...
def uid_exists(collection: Collection, uid: int) -> bool:
    """Надёжная проверка по PK через query‑expr (чуть медленнее, но без ошибок версии)."""
    try:
        res = collection.query(expr=f"id == {uid}", output_fields=["id"], consistency_level="Strong")
        return bool(res)
    except Exception as exc:
        # лог опционально, но не пропускаем дубликат по молчанию
        print(f"[uid_exists] Milvus error: {exc}; fallback = True")
        return True
...
async def load_data() -> list[dict]:
    async with aiofiles.open(INPUT_PATH, "r", encoding="utf-8") as f:
        return json.loads(await f.read())[:1000]
