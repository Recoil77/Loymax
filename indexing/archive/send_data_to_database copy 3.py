...
# v2.4 — добавлено поле `embedding_text` (VARCHAR 8192) и сохранение очищенного текста.
...
def uid_exists(collection: Collection, uid: int) -> bool:
    try:
        ...
        return True  # безопасный вариант — считаем, что есть
...
async def process_item(item: dict, collection: Collection, session: aiohttp.ClientSession,
    ...
    async with sem:
        ...
        try:
            clean_text = await get_embedding_text(session, text)  # очищенная версия
